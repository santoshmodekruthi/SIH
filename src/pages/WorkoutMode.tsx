...
export const WorkoutMode = () => {
  ...
  const workout = exercises[0]; // mock: just picking the first one
  ...
  return (
    <div className="min-h-screen bg-background flex flex-col">
      ...
      <div className="flex items-center justify-between p-6 absolute top-0 w-full z-50 bg-gradient-to-b from-background to-transparent">
        <button onClick={() => navigate(-1)} className="p-2 rounded-full bg-surface/80 hover:bg-surface border border-surfaceHover backdrop-blur transition-colors">
          <X className="w-6 h-6" />
        </button>
          ...
          {Array.from({ length: workout.sets }).map((_, i) => (
            <div 
              ...
              className={`h-2 w-8 rounded-full transition-colors ${
                ...
                i + 1 === currentSet ? 'bg-primary/50' : 'bg-surfaceHover'
              }`}
            />
          ))}
        </div>
        ...
        <div className="flex-1 flex flex-col justify-center p-8 lg:p-16 z-10 -mt-20 lg:mt-0">
          <AnimatePresence mode="wait">
            {!isResting ? (
              ...
              >
                <div>
                  <p className="text-primary font-bold tracking-wider mb-2 uppercase text-sm">Set {currentSet} / {workout.sets}</p>
                  <h2 className="text-4xl md:text-5xl font-bold mb-4">{workout.name}</h2>
                  <p className="text-6xl font-black text-transparent bg-clip-text bg-gradient-to-r from-white to-muted">
                    {workout.reps} <span className="text-3xl text-muted">REPS</span>
                  </p>
                </div>

                <div className="space-y-4">
                  <h3 className="font-semibold text-muted">Instructions:</h3>
                  <ul className="space-y-2">
                    {workout.instructions.map((inst, idx) => (
                ...
                <Button size="lg" fullWidth className="h-16 text-xl mt-8" onClick={handleCompleteSet}>
              ...
              >
                <p className="text-primary font-bold tracking-wider uppercase text-xl">Rest</p>
                ...
                <div className="pt-8 border-t border-surfaceHover">
                  <p className="text-muted mb-2">Up Next</p>
                  <p className="font-semibold text-lg">{workout.name} (Set {currentSet + 1})</p>
                </div>
              </motion.div>
            )}
          </AnimatePresence>
